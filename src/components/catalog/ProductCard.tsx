...
import NotifyModal from '@/components/drop/NotifyModal';
                ...
                <div className={`relative overflow-hidden bg-black/50 ${viewMode === 'list' ? 'w-48 shrink-0' : 'aspect-[4/5]'}`}>
                    ...
                    {viewMode === 'grid' && (
                        <div className="absolute bottom-4 left-0 right-0 px-4 flex gap-2 translate-y-full group-hover:translate-y-0 transition-transform duration-300 z-20">
                            <Button
                                className={`flex-1 font-bold ${isOutOfStock
                                    ? 'bg-mtrix-gray text-white hover:bg-mtrix-gray/80'
                                    : 'bg-white text-black hover:bg-primary hover:text-black'}`}
                                onClick={isOutOfStock ? handleNotify : handleAddToCart}
                            >
                                {isOutOfStock ? (
                                    <>
                ...
                <CardContent className={`p-4 flex flex-col justify-between ${viewMode === 'list' ? 'flex-1 py-4' : ''}`}>
                    ...
                    {viewMode === 'list' && (
                        <div className="flex gap-3 mt-auto">
                            <Button
                                className={`font-bold px-8 ${isOutOfStock
                                    ? 'bg-mtrix-gray text-white hover:bg-mtrix-gray/80'
                                    : 'bg-white text-black hover:bg-primary hover:text-black'}`}
                                onClick={isOutOfStock ? handleNotify : handleAddToCart}
                            >
